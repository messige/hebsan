    ...
    setup () {
      ...
      for (i = 0 ; i < calendarArraySorted.length ; i++ ) { 
        if ( compareDesc ( calendarArraySorted[i].calendarDate  , today ) > 0   
          ...
          feastIndex  = originalString.replace(' ', '') ;
          feastIndex  = feastIndex.replace(' ', '') ;   // we should use a recursive function : see https://www.geeksforgeeks.org/remove-all-occurrences-of-a-character-in-a-string-recursive-approach/
          feastIndex  = feastIndex.replace(' ', '') ;   // but how ???
          feastIndex  = feastIndex.replace(' ', '') ;
          feastIndex  = feastIndex.replace(' ', '') ;
