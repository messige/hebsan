<template>
<!-- - keyCalendar = 201 - keyPiece = 5302 - keyText = 5438 -->
 <template v-if=" annoABC == 'A' ||  annoABC == 'B' ||  annoABC == 'C'">
<rubrique>Introitus</rubrique><br /><br />Iustus ut palma florebit,<br />
sicut cedrus Libani multiplicabitur.
<ion-grid><ion-row><ion-col>&nbsp;</ion-col><ion-col size="11">
<ion-text color="tertiary">Der Gerechte wird wie eine Palme blühen,<br />
wie eine Zeder auf dem Libanon wird er wachsen.</ion-text></ion-col></ion-row></ion-grid>

 <!-- textType = C keyText = 7138 --> plantatus in domo Domini,<br />
in atriis domus Dei nostri.
<ion-grid><ion-row><ion-col>&nbsp;</ion-col><ion-col size="11"><ion-text color="tertiary">Gepflanzt im Hause des Herrn,<br />
in den Höfen des Hauses unseres Gottes.</ion-text></ion-col></ion-row></ion-grid>
 <!-- textType = P keyText = 3509 -->Ps. Et psallere nomini tuo, altissime.
<ion-grid><ion-row><ion-col>&nbsp;</ion-col><ion-col size="11"><ion-text color="tertiary">Und deinem Namen zu lobsingen, du Höchster.</ion-text></ion-col></ion-row></ion-grid>
</template>
<!-- - keyPiece = 2611 - keyText = 2669 -->
 <template v-if=" annoABC == 'A' ||  annoABC == 'B' ||  annoABC == 'C'">
<rubrique>Graduale</rubrique><br /><br />Domine, praevenisti eum <br />
in benedictionibus dulcedinis :<br />
posuisti in capite eius <br />
coronam de lapide pretioso.
<ion-grid><ion-row><ion-col>&nbsp;</ion-col><ion-col size="11">
<ion-text color="tertiary">Herr, du kamst ihm zuvor <br />
mit gütigem Segen;<br />
du hast ihm auf sein Haupt gesetzt <br />
eine Krone von Edelstein.</ion-text></ion-col></ion-row></ion-grid>

 <!-- textType = V keyText = 10030 -->V./  Vitam petiit,<br />
et tribuisti ei longitudinem dierum <br>in saeculum saeculi.
<ion-grid><ion-row><ion-col>&nbsp;</ion-col><ion-col size="11"><ion-text color="tertiary">Leben erbat er von dir,<br />
und du gabst ihm Länge der Tage <br>in alle Ewigkeit.</ion-text></ion-col></ion-row></ion-grid>
</template>
<!-- - keyPiece = 1065 - keyText = 1095 -->
 <template v-if=" annoABC == 'A' ||  annoABC == 'B' ||  annoABC == 'C'">
<rubrique>Tractus - canticus</rubrique><br /><br />Beatus vir qui timet Dominum,<br />
in mandatis eius volet nimis.
<ion-grid><ion-row><ion-col>&nbsp;</ion-col><ion-col size="11">
<ion-text color="tertiary">Selig der Mann, der den Herrn fürchtet,<br />
an seinen Geboten freut er sich sehr.</ion-text></ion-col></ion-row></ion-grid>

 <!-- textType = V keyText = 4090 -->V./  Gloria et divitiae in domo eius:<br />
et iustitia eius manet in saeculum saeculi.
<ion-grid><ion-row><ion-col>&nbsp;</ion-col><ion-col size="11"><ion-text color="tertiary">Herrlichkeit und Reichtum sind in seinem Haus,<br />
und seine Gerechtigkeit bleibt in alle Ewigkeit.</ion-text></ion-col></ion-row></ion-grid>
</template>
<!-- - keyPiece = 9549 - keyText = 9768 -->
 <template v-if=" annoABC == 'A' ||  annoABC == 'B' ||  annoABC == 'C'">
<rubrique>Offertorium</rubrique><br /><br />Veritas mea et misericordia mea cum ipso :<br />
et in nomine meo exaltabitur cornu eius.
<ion-grid><ion-row><ion-col>&nbsp;</ion-col><ion-col size="11">
<ion-text color="tertiary">Meine Wahrheit und mein Erbarmen werden mit ihm sein,<br />
und in meinem Namen wird sich sein Horn erheben.</ion-text></ion-col></ion-row></ion-grid>

</template>
<!-- - keyPiece = 5047 - keyText = 5193 -->
 <template v-if=" annoABC == 'A' ||  annoABC == 'B' ||  annoABC == 'C'">
<rubrique>Communio</rubrique><br /><br />Ioseph fili David noli timere<br />
accipere Mariam coniugem tuam :<br />
quod enim in ea <span class="fc">natum est</span> de Spiritu Sancto est.
<ion-grid><ion-row><ion-col>&nbsp;</ion-col><ion-col size="11">
<ion-text color="tertiary">Josef, Sohn Davids, fürchte dich nicht,<br />
Maria als deine Frau zu dir zu nehmen;<br />
was nämlich in ihr geboren wird, ist vom Heiligen Geist.</ion-text></ion-col></ion-row></ion-grid>

</template>
<!-- - keyPiece = 3727 - keyText = 3889 -->
 <template v-if=" annoABC == 'A' ||  annoABC == 'B' ||  annoABC == 'C'">
<rubrique>Communio</rubrique><br /><br />Fili, quid fecisti nobis sic?<br />
Ego et pater tuus dolentes quaerebamus te.<br />
Quid est quod me quaerebatis?<br />
Nesciebatis quia in his quae Patris mei sunt oportet me esse?
<ion-grid><ion-row><ion-col>&nbsp;</ion-col><ion-col size="11">
<ion-text color="tertiary">Kind, warum hast du uns das getan?<br />
Ich und dein Vater haben dich mit Schmerzen gesucht!<br />
Warum habt ihr mich gesucht?<br />
Wusstet ihr nicht, dass ich in dem sein muß, was meines Vaters ist?</ion-text></ion-col></ion-row></ion-grid>

</template>
 <br /></template>
 <script lang="ts">
 export default {
 props: {
 annoABC: {
 type: String,
 }
 }
 }
 </script>