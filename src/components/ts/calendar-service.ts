...
export default function ( thisDay: Date ) { 
      ...
      class CalendarItem {
        ...
        const pushItem = function ( obj: { calendarFeast: string ; calendarDate: Date ; calendarABC: string } ) { calendarArray.push ( obj ) ; } ;
        const dateFormat = function ( obj: {calendarFeast:  string ; calendarDate: Date ; calendarABC: string } ) { 
          const daysFR         = [  "Dimanche", 
                                    "Lundi\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0", 
                                    "Mardi\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0", 
                                    "Mercredi\u00A0\u00A0", "Jeudi\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0", 
                                    "Vendredi", "Samedi\u00A0\u00A0\u00A0\u00A0" ] ;
      ...
      if ( getDay ( new Date ( civilYear , 0, 1  )  )  == 0 ) {    // if 1st January is a Sunday, feast is Dei genitricis Maria 
      ...
      do {
        ...
        calendarObj     = new CalendarItem ( "Tempus per annum " + i , incrementDate , annoABC ) ;
        pushItem      ( calendarObj ) ;
        i++
      ...
      do {
        ...
        calendarObj     = new CalendarItem ( "Tempus quadragesimae " + i , incrementDate , annoABC ) ;
        pushItem      ( calendarObj ) ;
        i++ 
      ...
      for ( i = 1 ; i < 6 ; i++ ) { 
        ...
        calendarObj   = new CalendarItem ( "Tempus paschale " + (i+1) , incrementDate , annoABC ) ;
        pushItem      ( calendarObj ) }
      incrementDate     = addWeeks ( easter, i ) ;
      ...
      for ( i = 0 ; i < 4 ; i++ ) { 
        ...
        pushItem      ( calendarObj ) }
      i = 0 ;
      do {  incrementDate = addWeeks ( easter, i + 10 ) ;
        calendarObj   = new CalendarItem ( "Tempus per annum " + indexPerAnnum , incrementDate , annoABC ) ;
        pushItem      ( calendarObj ) ;               
        i++
        indexPerAnnum ++ ;
      ...
      for ( i = 1 ; i < 5 ; i++ ) { 
        ...
        calendarObj   = new CalendarItem ( "Tempus adventus " + i , incrementDate , annoABCPlus1 ) ;
        pushItem      ( calendarObj ) } 
      calendarObj   = new CalendarItem ( "In Nativitate Domini" , christmas , annoABCPlus1 ) ;
      pushItem      ( calendarObj ) ; 
      if ( getDay ( christmas )  > 0 ) {
        calendarObj   = new CalendarItem ( "Sanctae Familiae" , nextSunday ( christmas ) , annoABCPlus1 ) ;
        pushItem      ( calendarObj ) ;
      }
      ...
      if ( getDay ( presentatio ) == 0 || getDay ( iosephSponsus ) == 0 || getDay ( petriPauli ) == 0 || getDay ( bmConceptio ) == 0) {
        for ( const thisItem2 of calendarArray ) {
          if ( compareDesc ( thisItem2.calendarDate , presentatio ) == 0 )    { thisItem2.calendarFeast = "Presentatio Domini" }
          if ( compareDesc ( thisItem2.calendarDate , iosephSponsus ) == 0 )  { thisItem2.calendarFeast = "Ioseph sponsus" }
          if ( compareDesc ( thisItem2.calendarDate , petriPauli ) == 0 )     { thisItem2.calendarFeast = "Petri et Pauli" }
          if ( compareDesc ( thisItem2.calendarDate , bmConceptio ) == 0 )    { thisItem2.calendarFeast = "Beatae Mariae Virginis Conceptio" }
        }    
      }             
      const calendarArraySorted =           // see https://www.cloudhadoop.com/typescript-sort-object-array/
...
/*             Compare the two dates and return -1 if the first date is after the second,. 
            ...
            if ( compareDesc ( thisDay ,calendarArraySorted[j].calendarDate ) > -1 ) {
              if ( compareDesc ( calendarArraySorted[j].calendarDate , lastDate ) > 0 )  {
                ...
                calendarFrom [k]  =  { thisFeast:calendarArraySorted[j].calendarFeast ,
